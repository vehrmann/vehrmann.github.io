...
function updateModelViewer(total_duration) {
    //modelviewer.currentTime = modelviewer.currentTime
    modelviewer.currentTime = total_duration - 0.0000001
}
...
modelviewer.addEventListener('load', function() {
    ...
    slider_animation_korpus_updown.addEventListener('input', function() {
        action_z.timeScale = this.value / this.max
        updateModelViewer(action_z._clip.duration)
    });
    ...
    slider_animation_klappe_openclose.addEventListener('input', function() {
        action_y.timeScale = this.value / this.max
        updateModelViewer(action_z._clip.duration)
    });
    ...
    let action_z = registerAnimations('energiekette_r_armature')
    let action_y = registerAnimations('energiekette_r_fussAction')
